...
void animationHandler(int param)
{
	int i;
	for (i = 0; i < subcurve.numCurvePoints; i++) {
		distanceX = (subcurve.curvePoints[i + 1].x - subcurve.curvePoints[i].x);
		distanceZ = (subcurve.curvePoints[i + 1].y - subcurve.curvePoints[i].y);
		double m = sqrt(distanceX*distanceX + distanceZ*distanceZ);
		subcurve.curvePoints[i].x = distanceX / m;
		subcurve.curvePoints[i].x = -distanceZ / m;
	}
	glutPostRedisplay();
...
void drawBot() 
{
	...
	{
		glPushMatrix();
		glTranslatef(circles[0].circleCenter.x + distanceX, 0, -circles[0].circleCenter.y + distanceZ);
	...
	else if (botType == WHEEL)
	{
		...
		glTranslatef(circles[0].circleCenter.x, 0, -circles[0].circleCenter.y);
		glRotatef(angle, 0, 1, 0);

		glPushMatrix();
		glRotatef(90, 0, 1, 0);
		gluDisk(gluNewQuadric(), 0, 1.5, 20, 20);
		gluCylinder(gluNewQuadric(), 1.5, 1.5, 1.5, 20, 20);
		glPopMatrix();
...
void drawCannon() 
{
	...
	// The cone part of cannon
	glPushMatrix();
	glTranslatef(-5, 0, 11);
	glRotatef(-90, 1, 0, 0);
	glutSolidCone(1, 2.5, 30, 30);
	glPopMatrix();
	...
	glRotatef(45,0,1,0);
	glRotatef(90, 0, 1, 0);
	gluCylinder(gluNewQuadric(), 0.28, 0.28, 2, 20, 20);
	glPopMatrix();
